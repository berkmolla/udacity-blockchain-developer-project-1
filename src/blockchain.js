...
class Blockchain {
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (stars.length === 0) {
                reject("No stars found belonging to this address");
            }
            console.log(stars);
            var starBodies = [];
            for (const star of stars) {
                const dat = await star.getBData()
                starBodies.push(dat)
            }
            console.log(starBodies)
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (const [index, block] of self.chain.entries()) {
                try {
                    if (index !== 0) {
                        previousBlockHash = self.chain[index - 1].hash;
                        if (block.previousBlockHash !== previousBlockHash) {
                            errorLog.push({'index': index, 'error': 'Block previous hash does not match previous block hash'})
                        }
                    }
                    let isValid = await block.validate();
                } catch (error) {
                    errorLog.push({'index': index, 'error': error})
                }
            }
            resolve(errorLog);
